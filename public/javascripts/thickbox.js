...
function tb_show(caption, url, imageGroup) {//function called when the user clicks on a thickbox link
    url = voranet_url(url);
	clase = caption.toLowerCase().replace(/\s/g,'-');
	clase = clase.replace(/[^a-zA-Z0-9\-]/g,'');
	try {
		if (typeof document.body.style.maxHeight === "undefined") {//if IE 6
			$("body","html").css({height: "100%", width: "100%"});
			$("html").css("overflow","hidden");
			if (document.getElementById("TB_HideSelect") === null) {//iframe to hide select elements in ie6
				$("body").append("<iframe id='TB_HideSelect'></iframe><div id='TB_overlay'></div><div id='TB_window' class='"+clase+"'></div>");
				$("#TB_overlay").click(tb_remove);
			}
		}else{//all others
			if(document.getElementById("TB_overlay") === null){
				$("body").append("<div id='TB_overlay'></div><div id='TB_window' class='"+clase+"'></div>");
				$("#TB_overlay").click(tb_remove);
			}
		}
...
function tb_remove() {
	...
	if (typeof document.body.style.maxHeight == "undefined") {//if IE 6
		...
		//$("html").css("overflow","auto"); mod manuel para que aparezca bien el scroll vertical en IE6 despues de cerrar el tb_window
